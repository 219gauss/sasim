...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        filter = new IntentFilter("com.android.InCall.speaker");
        context.registerReceiver(mSpeakerReceiver, filter);

        filter = new IntentFilter("com.android.InCall.dialer.key.status");
        context.registerReceiver(mDialerKeyReceiver, filter);
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_M1:
                if(down) {
                    ...
                    if(!mTelecomManager.isRinging() || !mTelecomManager.isInCall()) {
                        ...
                        if(!telNumStr.equals("sasin0123456789") && telNumStr.length() >= 3) {
                            Intent intent = new Intent(Intent.ACTION_CALL);
            ...
            case KeyEvent.KEYCODE_M2:
                if(down) {
                    Log.e(TAG, "KEYCODE_M2 is down");
                    if(!mTelecomManager.isRinging() || !mTelecomManager.isInCall()) {
                        String telNumStr = Settings.Global.getString(mContext.getContentResolver(), Settings.Global.SASINCOMM_SPEED_DIAL_M2);
                        Log.e(TAG, "telNumStr : "+telNumStr);
                        if(!telNumStr.equals("sasin0123456789") && telNumStr.length() >= 3) {
                            Intent intent = new Intent(Intent.ACTION_CALL);
                            intent.setData(Uri.parse("tel:" + telNumStr));//设置数据 直接拨打电话
            ...
            case KeyEvent.KEYCODE_M3:
                if(down) {
                    Log.e(TAG, "KEYCODE_M3 is down");
                    if(!mTelecomManager.isRinging() || !mTelecomManager.isInCall()) {
                        String telNumStr = Settings.Global.getString(mContext.getContentResolver(), Settings.Global.SASINCOMM_SPEED_DIAL_M3);
                        Log.e(TAG, "telNumStr : "+telNumStr);
                        if(!telNumStr.equals("sasin0123456789") && telNumStr.length() >= 3) {
                            Intent intent = new Intent(Intent.ACTION_CALL);
                            intent.setData(Uri.parse("tel:" + telNumStr));//设置数据 直接拨打电话
            ...
            case KeyEvent.KEYCODE_M4:
                if(down) {
                    Log.e(TAG, "KEYCODE_M4 is down");
                    if(!mTelecomManager.isRinging() || !mTelecomManager.isInCall()) {
                        String telNumStr = Settings.Global.getString(mContext.getContentResolver(), Settings.Global.SASINCOMM_SPEED_DIAL_M4);
                        Log.e(TAG, "telNumStr : "+telNumStr);
                        if(!telNumStr.equals("sasin0123456789") && telNumStr.length() >= 3) {
                            Intent intent = new Intent(Intent.ACTION_CALL);
                            intent.setData(Uri.parse("tel:" + telNumStr));//设置数据 直接拨打电话
    ...
    //add by pzp
    BroadcastReceiver mSpeakerReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if("com.android.InCall.speaker".equals(intent.getAction())){
                Log.e(TAG,"mSpeakerReceiver action receiver !!!");
                String data  = intent.getStringExtra("extra_data");
                if(data.equals("open_handfree")) {
                    flag = 1;
                } else if(data.equals("close_handfree")) {
                    flag = 0;
                }
            }
        }
    ...
    private boolean dialer_intface = false;
    BroadcastReceiver mDialerKeyReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if("com.android.InCall.dialer.key.status".equals(intent.getAction())){
                Log.e(TAG,"mDialerKeyReceiver action receiver !!!");
                String data  = intent.getStringExtra("extra_data");
                if(data.equals("dialer_intface_open")) {
                    dialer_intface = true;
                } else if(data.equals("dialer_intface_close")) {
                    dialer_intface = false;
                }
            }
        }
    };
    ...
    private final String LedControlPath = "/sys/class/sasin_gpios/sasin_gpio92/ledctrl";
    class LedControlRunable implements Runnable {
        ...
        public void run() {
            while(ledRun) {
                restoreDeviceValue(new File(LedControlPath), ledflag ? "0x1" : "0x0");
                ledflag = !ledflag;
                try {
                    Thread.sleep(1000);
            ...
            restoreDeviceValue(new File(LedControlPath), "0x0");
        }
    }
    ...
    public boolean getHookStatus(){
        String value = "sys/class/sasin_gpios/sasin_gpio92/state";
        try {
                ...
                br.close();
                return valueString != null && valueString.equals("0");//此处1表示灯亮，0表示灯灭
        } catch (IOException e) {
